...
模板里用一对双大括号包裹着组件类 `hi` 属性，你可以随时改变 `hi` 属性的值（这里设置了一个定时器3秒后改变值），Angular会实时同步更新模板的渲染。
...
如果你很细心不知是否发现，模板中会用 `两对大括号`、`()` 来表示一些特定的行为能力，这其实是Angular的一种数据流向的约定，这一点在[模板语法-绑定语法](../template/binding-syntax.md)会详细介绍。